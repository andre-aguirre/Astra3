...
The hero draws the Cα trace in `src/lib/trace-data.json`, currently adenylate kinase (PDB 4AKE). To use another structure, point the generator at any PDB-format file, for example a `.pdb` from an Astra3 run's `Session/` folder, and update the caption in `src/components/Hero.astro` to match:

```
node scripts/make-trace.mjs path/to/1ERJ.pdb C "PDB 1ERJ · TUP1 WD40 DOMAIN"
```
